...
class Solution(object):
    def maxSubArray(self, nums):
        ...
        if len(nums)==0:
            ...
            maxsum=nums[0]
            maxarray=nums[0:2]
            id=list(itertools.permutations(range(0,len(nums)+1), 2))
            #print id
            for i in range(0,len(id)):
                if id[i][0]<id[i][1]:
                    subarray = nums[id[i][0]:id[i][1]]
                    subsum = sum(subarray)
                    if subsum > maxsum:
                        maxsum = subsum
                        maxarray = subarray
