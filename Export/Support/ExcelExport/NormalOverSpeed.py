...
class NormalOverSpeed(ExcelExportBase):
    ...
    def __init__(self, logObj, startDate, endDate, vehicleType, platformId,  govStatus, FuncVType):
        super(NormalOverSpeed, self).__init__(logObj, startDate, endDate, vehicleType, platformId, govStatus, FuncVType) #super继承
    ...
    def ExportSum(self):
        ...
        if (VTypeStrSplit(self.FuncVType,4)!=0):
            s=s+'超4小时运行'
        #s=s[:-1]
        workbook = self.CreateWorkbook('全省卫星定位系统车辆动态统计报表(%d-%d_危险品%s).xlsx'
        ...
        if (VTypeStrSplit(self.FuncVType,1)!=0):
            ...
            wsAll.merge_range(3, 5, 3, 8, '超速', self.centerFormat)
            wsAll.merge_range(4, 5, 5, 5, '违章车辆数', self.centerFormat)
            wsAll.merge_range(4, 6, 5, 6, '违章车辆比例', self.centerFormat)
            wsAll.merge_range(4, 7, 5, 7, '高速≥20%', self.centerFormat)
            wsAll.merge_range(4, 8, 5, 8, '非高速≥50%', self.centerFormat)            
        else:
            i=i+4
        if (VTypeStrSplit(self.FuncVType,2)!=0):
            wsAll.set_column(10-i, 10-i, 6)
            wsAll.set_column(11-i, 11-i, 10)
            wsAll.merge_range(3, 9-i, 3, 11-i, '凌晨2-5点违规运行', self.centerFormat)                    
            wsAll.merge_range(4, 9-i, 5, 9-i, '违章车辆数', self.centerFormat)
            wsAll.merge_range(4, 10-i, 5, 10-i, '次数', self.centerFormat)
            wsAll.merge_range(4, 11-i, 5, 11-i, '违章车辆比例', self.centerFormat)
        else:
            i=i+3
        if (VTypeStrSplit(self.FuncVType,3)!=0):       
            wsAll.set_column(13-i, 13-i, 6)
            wsAll.set_column(14-i, 14-i, 10)
            wsAll.merge_range(3, 12-i, 3, 14-i, '持续半小时无数据上传', self.centerFormat)
            wsAll.merge_range(4, 12-i, 5, 12-i, '违章车辆数', self.centerFormat)
            wsAll.merge_range(4, 13-i, 5, 13-i, '次数', self.centerFormat)
            wsAll.merge_range(4, 14-i, 5, 14-i, '违章车辆比例', self.centerFormat)
        else:
            i=i+3
        if (VTypeStrSplit(self.FuncVType,4)!=0):
            wsAll.set_column(16-i, 16-i, 6)
            wsAll.set_column(17-i, 17-i, 10)
            wsAll.merge_range(3, 15-i, 3, 17-i, '超4小时运行', self.centerFormat)
            wsAll.merge_range(4, 15-i, 5, 15-i, '违章车辆数', self.centerFormat)
            wsAll.merge_range(4, 16-i, 5, 16-i, '次数', self.centerFormat)
            wsAll.merge_range(4, 17-i, 5, 17-i, '违章车辆比例', self.centerFormat)
        else:
            i=i+3
        ...
        wsAll.write('A1', '附件：')
        wsAll.merge_range(1, 0, 1, 18-i, '全省卫星定位系统危险货物运输车辆动态统计报表', self.titleFormat)
        wsAll.write('A3', timeSpan)
        wsAll.write(2, 18-i, '单位：辆/次', self.centerNoBorder)
        wsAll.merge_range(3, 0, 5, 0, '地区', self.centerFormat)
        wsAll.merge_range(3, 1, 5, 1, '车辆应安装总数', self.centerFormat)
        wsAll.merge_range(3, 2, 5, 2, '车辆安装总数', self.centerFormat)
        wsAll.merge_range(3, 3, 5, 3, '入网率', self.centerFormat)
        wsAll.merge_range(3, 4, 5, 4, '上线率', self.centerFormat)

        wsAll.merge_range(3, 18-i, 5, 18-i, '违章行为次数合计', self.centerFormat)
        ...
        for zoneId in zoneIdList:
            ...
            if (VTypeStrSplit(self.FuncVType,1)!=0):
                ...
                wsZone.merge_range(3, 4, 3, 7, '超速', self.centerFormat)
                wsZone.merge_range(4, 4, 5, 4, '违章车辆数', self.centerFormat)
                wsZone.merge_range(4, 5, 5, 5, '违章车辆比例', self.centerFormat)
                wsZone.merge_range(4, 6, 5, 6, '高速≥20%', self.centerFormat)
                wsZone.merge_range(4, 7, 5, 7, '非高速≥50%', self.centerFormat)
            else:
                j=j+4
            if (VTypeStrSplit(self.FuncVType,2)!=0):
                ...
                wsZone.merge_range(3, 8-j, 3, 10-j, '凌晨2-5点违规运行', self.centerFormat)
                wsZone.merge_range(4, 8-j, 5, 8-j, '违章车辆数', self.centerFormat)
                wsZone.merge_range(4, 9-j, 5, 9-j, '次数', self.centerFormat)
                wsZone.merge_range(4, 10-j, 5, 10-j, '违章车辆比例', self.centerFormat)
            else:
                j=j+3
            if (VTypeStrSplit(self.FuncVType,3)!=0):
                ...
                wsZone.merge_range(3, 11-j, 3, 13-j, '持续半小时无数据上传', self.centerFormat)
                wsZone.merge_range(4, 11-j, 5, 11-j, '违章车辆数', self.centerFormat)
                wsZone.merge_range(4, 12-j, 5, 12-j, '次数', self.centerFormat)
                wsZone.merge_range(4, 13-j, 5, 13-j, '违章车辆比例', self.centerFormat)
            else:
                j=j+3
            if (VTypeStrSplit(self.FuncVType,4)!=0):
                ...
                wsZone.merge_range(3, 14-j, 3, 16-j, '超4小时运行', self.centerFormat)
                wsZone.merge_range(4, 14-j, 5, 14-j, '违章车辆数', self.centerFormat)
                wsZone.merge_range(4, 15-j, 5, 15-j, '次数', self.centerFormat)
                wsZone.merge_range(4, 16-j, 5, 16-j, '违章车辆比例', self.centerFormat)
            else:
                j=j+3
            ...
            wsZone.write('A1', '附件：')
            wsZone.merge_range(1, 0, 1, 17-j, '%s 卫星定位系统危险货物运输车辆动态统计报表'
                               %self.GetZoneName(zoneId), self.titleFormat)
            wsZone.write('A3', timeSpan)
            wsZone.write(2, 17-j, '单位：辆/次', self.centerNoBorder)
            ...
            wsZone.merge_range(3, 17-j, 5, 17-j, '违章行为次数合计', self.centerFormat)
    ...
    def ExportVehicle(self):
        ...
        workbook = self.CreateWorkbook('危险品车辆违法六严禁汇总表.xlsx'.encode(ExcelExportConfig.envEncode))
        ...
        wsVehicle.merge_range(0, 0, 0, 11-m, '危险品车辆违法六严禁汇总表', titleFormat)
